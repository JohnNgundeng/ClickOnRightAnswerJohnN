...
local Sound1 = audio.loadSound("Sounds/PlugWalk.mp3") 
local Sound2 = audio.loadSound("Sounds/WrongBuzzer.mp3") 
local Sound3 = audio.loadSound("Sounds/CorrectAnswer.mp3") 
...
local function TouchListenerAnswer(touch)
    ...
    if (touch.phase == "ended") and (alreadyClickedAnswer == false) then
        ...
        if (answer == tonumber(userAnswer)) then     
            ...
            -- making sound for right answer
            rightsound = audio.play(Sound3)
...
local function TouchListenerWrongAnswer1(touch)
    ...
    if (touch.phase == "ended") and (alreadyClickedAnswer == false) then
        ...
        if (answer ~= tonumber(userAnswer)) then
            ...
            -- decrease a life
            lives = lives - 1
            -- creating the sound if the get it wrong
            wrongsound = audio.play(Sound2)
            -- call RestartScene after 1 second
            timer.performWithDelay( 1000, RestartScene )            
        end        
    ...
    local userAnswer = wrongAnswer2TextObject.text
        ...
        if (touch.phase == "ended") and (alreadyClickedAnswer == false) then
            ...
            if (answer ~= tonumber(userAnswer)) then
                ...
                -- decrease a life
                lives = lives - 1
                -- creating the sound when they get it wrong
                wrongsound = audio.play(Sound2)
                -- call RestartScene after 1 second
                timer.performWithDelay( 1000, RestartScene )            
            end        
...
function scene:create( event )
    ...
    incorrect:setTextColor(210/255, 47/255, 100/255)
    incorrect.isVisible = false
